<script>
    ...
    const makeCells = (data) => {
        const sortIndex = Array.from(data.keys()).sort(
            (a, b) => data[a] - data[b],
        );
        const reverseIndex = Array.from(data.keys()).map((i) =>
            sortIndex.indexOf(i),
        );
    ...
    function positionLabels(selectedData, labels, container) {
        ...
        if (
            selectedData &&
            container &&
            filteredLabels.length === selectedData.length
        ) {
            ...
            for (let i = 0; i < sortedLabels.length; i++) {
                ...
                if (
                    ...
                    (labelLeft - containerLeft < containerRight - labelRight ||
                        labelRight >= containerRight)
                )
                    lab.align = "left";
            ...
            return {
                labels: selectedData.map((d) =>
                    positionedLabels.find((p) => p.areacd === d.areacd),
                ),
                maxOffset,
            };
        } else {
...
>
    {#each breaks.slice(1) as brk, i}
        ...
        <div
            class="tick"
            style="left: {i *
                (100 / (breaks.length - 1))}%; transform: translateX({i == 0 &&
            snapTicks
                ? '-2px'
                : '-50%'});"
        >
    ...
    <div
        class="tick"
        style="right: 0; transform: translateX({snapTicks ? '2px' : '50%'});"
    >
    ...
    {#if Array.isArray(positionedData?.labels)}
        {#each positionedData.labels as d, i}
            <div
                style:opacity={d.areacd === hovered
                    ? null
                    : hovered
                      ? "30%"
                      : null}
