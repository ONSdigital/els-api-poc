<script lang="ts">
    ...
    function nearestPoint(points, bb) {
        ...
        for (const ft of filtered) {
            if (
                dist(centroid, ft.geometry.coordinates) <
                dist(centroid, nearest.geometry.coordinates)
            )
                nearest = ft;
        }
        return nearest;
    ...
    function filterPoints(points, polygon) {
        ...
        let filtered = points.features.filter(
            (ft) => inPoly(ft, bbPoly) && inPoly(ft, polygon),
        );
