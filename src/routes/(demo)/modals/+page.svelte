<script lang="ts">
  ...
  import { makePeriodFormatter } from "$lib/utils";
  ...
  let pageState = $state({
    ...
    selectedGeoLevel: data.geoLevels.find(
      (g) => g.id === data.metadata.geography.initialLevel,
    ),
    selectedPeriodRange: [
      data.periods[0],
...
<Section>
  ...
  <p>
    Date range:
    <strong
      >{formatPeriod(pageState.selectedPeriodRange[0])} to {formatPeriod(
        pageState.selectedPeriodRange[1],
      )}</strong
    >
  </p>
  <p>
